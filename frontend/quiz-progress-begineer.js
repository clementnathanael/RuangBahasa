document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch(`https://ruangbahasa-backend.vercel.app/quiz-progress?username=${username}&quiz_id=${quizId}`);
        ...
        tryAgainButton.addEventListener('click', async () => {
            ...
            await fetch(`https://ruangbahasa-backend.vercel.app/reset-quiz?username=${username}&quiz_id=${quizId}`, { method: 'POST' });
            window.location.href = 'begineer.html';
        });
    } catch (error) {
